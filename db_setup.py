...
class AgendamentoComando(Base):
    __tablename__ = 'agendamento_comandos'
    id = Column(Integer, primary_key=True)
...
class ComandoRecorrente(Base):
    __tablename__ = 'comandos_recorrentes'
    id = Column(Integer, primary_key=True)
    usuario = Column(String(100), nullable=True)
...
class RelogioGrupo(Base):
    __tablename__ = 'relogio_grupos'
    id = Column(Integer, primary_key=True)
    nome = Column(String(100), unique=True, nullable=False)
    relogios = Column(Text, nullable=False, default='[]')  # JSON array string: ex. '[1, 11, 23, 29]'
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
    updated_at = Column(DateTime, default=datetime.datetime.utcnow, onupdate=datetime.datetime.utcnow)


INITIAL_CLOCK_GROUPS = {
    "P10": [1, 11, 23, 29],
    "COCA": [3, 14, 31],
...
def seed_initial_data(session):
    ...
    # 5. Seed Relógio Grupos (Mapeamento Inicial)
    try:
        for grupo_nome, ids in INITIAL_CLOCK_GROUPS.items():
            exists = session.query(RelogioGrupo).filter_by(nome=grupo_nome).first()
            if not exists:
                session.add(RelogioGrupo(nome=grupo_nome, relogios=json.dumps(ids)))
        session.commit()
        print("Relógio Grupos seeded statically.")
    except Exception as e:
        session.rollback()
        print(f"Error seeding Relógio Grupos: {e}")
